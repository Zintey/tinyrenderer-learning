...
struct vec {
    ...
    vec(const vec& v) { for (int i = 0; i < n; i++) data[i] = v.data[i]; }
    vec(vec&& v) noexcept { for (int i = 0; i < n; i++) data[i] = std::move(v.data[i]); }
    vec& operator=(const vec& v) { for (int i = 0; i < n; i++) data[i] = v.data[i]; return *this; }
    vec& operator=(vec&& v) noexcept { for (int i = 0; i < n; i++) data[i] = std::move(v.data[i]); return *this; }
...
template<typename T> struct vec<2,T> {
    ...
    vec<2,T>(vec<2,T>&& v) noexcept : x(std::move(v.x)), y(std::move(v.y)) {}
    template<typename U>
    ...
    vec<2,T>& operator=(vec<2,T>&& v) noexcept { x = std::move(v.x); y = std::move(v.y); return *this; }
...
template<typename T> struct vec<3, T> {
    ...
    vec<3,T>(vec<3,T>&& v) noexcept : x(std::move(v.x)), y(std::move(v.y)), z(std::move(v.z)) {}
    ...
    vec<3,T>& operator=(vec<3,T>&& v) noexcept { x = std::move(v.x); y = std::move(v.y); z = std::move(v.z); return *this; }
...
template<typename T> struct vec<4, T> {
    ...
    vec<4,T>(vec<4,T>&& v) noexcept : x(std::move(v.x)), y(std::move(v.y)), z(std::move(v.z)), w(std::move(v.w)) {}
    ...
    vec<4,T>& operator=(vec<4,T>&& v) noexcept { x = std::move(v.x); y = std::move(v.y); z = std::move(v.z); w = std::move(v.w); return *this; }
...
struct Transform {
    ...
    vec3f scale = {1, 1, 1};
    Transform() = default;
    Transform (vec3f position_, vec3f rotation_, vec3f scale_)
